...
<div class="card">
    <div class="card-header">
        <i class="fas fa-edit me-2"></i> Edit Purchase Order: {{ $purchaseOrder->po_number }}
        <span class="badge bg-{{ $purchaseOrder->getStatusBadgeColor() }} ms-2">
            {{ $purchaseOrder->getStatusLabel() }}
        </span>
    </div>
    <div class="card-body">
        <form action="{{ route('admin.purchase-orders.update', $purchaseOrder) }}" method="POST" id="po-form">
            @csrf
            @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('supplier_id') is-invalid @enderror" 
                            ...
                            @foreach($suppliers as $id => $name)
                                <option value="{{ $id }}" {{ old('supplier_id', $purchaseOrder->supplier_id) == $id ? 'selected' : '' }}>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('warehouse_id') is-invalid @enderror" 
                            ...
                            @foreach($warehouses as $id => $name)
                                <option value="{{ $id }}" {{ old('warehouse_id', $purchaseOrder->warehouse_id) == $id ? 'selected' : '' }}>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('order_date') is-invalid @enderror" 
                               id="order_date" name="order_date" value="{{ old('order_date', $purchaseOrder->order_date->format('Y-m-d')) }}" required>
                        @error('order_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="mb-3">
                ...
                <input type="date" class="form-control @error('expected_date') is-invalid @enderror" 
                       id="expected_date" name="expected_date" value="{{ old('expected_date', $purchaseOrder->expected_date ? $purchaseOrder->expected_date->format('Y-m-d') : '') }}">
                @error('expected_date')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
                ...
                <table class="table table-bordered" id="items-table">
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td colspan="2"><span id="subtotal">{{ number_format($purchaseOrder->subtotal, 2) }}</span></td>
                        </tr>
                        <tr>
                            <td colspan="3" class="text-end"><strong>Tax (5%):</strong></td>
                            <td colspan="2"><span id="tax">{{ number_format($purchaseOrder->tax_amount, 2) }}</span></td>
                        </tr>
                        <tr>
                            <td colspan="3" class="text-end"><strong>Grand Total:</strong></td>
                            <td colspan="2"><span id="grand-total">{{ number_format($purchaseOrder->grand_total, 2) }}</span></td>
                        </tr>
                    </tfoot>
                </table>
            </div>
